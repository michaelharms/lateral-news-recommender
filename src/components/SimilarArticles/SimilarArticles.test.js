...
test("renders all articles", () => {
  const { queryAllByTestId } = render(<SimilarArticles articles={articles} />);
  const articleElements = queryAllByTestId("similar-article");
  expect(articleElements.length).toEqual(articles.length);
});
